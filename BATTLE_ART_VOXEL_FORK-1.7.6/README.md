...
The v1.7.7 profiling result showed a useful split: CPU/hook overhead dropped
sharply, but the ~3 Hz 50 ms stalls remained. That cadence exactly matches
Gen1Recomp's 20-logic-frame water/flower animation boundary.

v1.7.8 removes the remaining runtime GPU upload entirely. Animated terrain
states are built once as immutable textures; the live render path only picks
the already-created texture for the current animation step. This avoids
`replacePixels()` while the game is running.


## v1.7.7 performance fix

This build includes a stutter-focused renderer patch based on a 10-second
Gen1Recomp Performance Monitor capture. The old animated terrain atlas path
updated the same GPU texture three times per second and the voxel scene repeatedly
resolved the same map palette/atlas dozens of times per frame. v1.7.7 double-buffers
animated atlases, performs dirty-tile uploads on the inactive texture, coalesces
per-frame palette lookups, and reduces temporary Lua matrix/table allocations.

The intended result is the same visual output with substantially less periodic
frame-time and garbage-collector pressure. No save migration is required.

...
## 1.7.8 - eliminate runtime animation uploads

- Replaced runtime `Image:replacePixels()` terrain-animation updates with
  immutable prebuilt animation atlases.
- Vanilla water + flower animation resolves to only 8 prebuilt states.
- During gameplay, an animation step now changes only the selected texture
  reference; there is no GPU texture mutation on the 20-frame (~3 Hz) boundary.
- This directly targets the remaining 47-72 ms stalls seen every ~0.333 s in
  the v1.7.7 Performance Monitor report.
- Removed the v1.7.7 double-buffer path, which reduced hook CPU but did not
  remove the periodic driver/upload stall.
- Custom animated tiles are supported up to 64 unique combined states / 4096
  logic frames per cycle; pathological larger definitions fall back to the
  static atlas rather than reintroducing runtime stutter.
- Keeps the v1.7.7 palette/TOD coalescing, matrix allocation reductions and
  reusable scene buffers.

# Changelog

## 1.7.7 - performance / stutter fix

- Fixed the recurring ~3 Hz terrain-animation stall seen in profiling.
- Animated terrain atlases now use two GPU textures and update the inactive one
  before swapping, avoiding synchronization against the texture currently in use.
- Animated atlas uploads now update only dirty 8x8 tile slots when the backend
  supports region replacement; a compatibility fallback keeps the old full upload
  on the inactive texture.
- Coalesced terrain palette/atlas resolution to once per map per rendered frame.
  This removes dozens of redundant `map.palette` / `world.tod` traversals per frame.
- Memoized `DayNight.tod()` for identical clock values.
- Replaced hot-path billboard/figure/caster matrix chains with algebraically
  equivalent one-allocation constructors.
- Reused pose, neighbour mesh and water-draw buffers to reduce Lua GC pressure.
- Preserves the original mod id and visual feature set.

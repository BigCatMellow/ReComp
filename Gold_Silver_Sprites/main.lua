...
advancedPack.palettes.MEW_SILVER_PALETTE = {{255, 255, 255},  {255,123,255}, {57,90,214},  {0, 0, 0}, }
    ...
    mod.options:define({
        ...
        { key = "trainerColorMode", label = "SPRITE TRAINER", type = "choice",
          default = "truecolor",
          choices = {
            { "TRUE COLOR", "truecolor" },
            { "GRAYSCALE", "grayscale" },
          } },
        { key = "animColorMode", label = "SPRITE ANIM", type = "choice",
          default = "truecolor",
          choices = {
            { "TRUE COLOR", "truecolor" },
            { "GRAYSCALE", "grayscale" },
          } },
        { key = "custBattleSprites", label = "CUST. B.SPRITES", type = "choice",
          default = "off",
          choices = {
            { "DEFAULT", "off" },
            { "ORIGINAL", "original" },
  ...
  -- CUST. B.SPRITES -- 
    ...
    local CUST_BACK_SPRITES = {
    off  = "assets/red_back.png",
    alt1 = "assets/red_back_alt1.png",
    alt2 = "assets/red_back_alt2.png",
  }
  local CUST_BACK_SPRITES_GRAY = {
    off  = "assets/bw/red_back.png",
    alt1 = "assets/bw/red_back_alt1.png",
    alt2 = "assets/bw/red_back_alt2.png",
  }

  for _, relPath in pairs(CUST_BACK_SPRITES) do
    mod.content.battle_sprite_scales:register(relPath, {
      path = mod.assets:path(relPath),
      scale = 1.0,
    })
  end
  for _, relPath in pairs(CUST_BACK_SPRITES_GRAY) do
    mod.content.battle_sprite_scales:register(relPath, {
      path = mod.assets:path(relPath),
      scale = 1.0,
    })
  ...
  mod.hooks:wrap("player.sprite", function(next, path, ctx)
    if ctx.side == "back" and not ctx.demo and not ctx.oakDemo then
      local pick = mod.options:get("custBattleSprites") or "off"
      if pick == "original" then
        -- pass the merged path straight through, untouched -- whatever
        -- another installed back-sprite mod (or the base game) resolved
        -- it to, ctx.trueColor included, so that mod's own art and color
        -- handling apply exactly as if we weren't installed
        return next(path, ctx)
      end
      local gray = mod.options:get("trainerColorMode") == "grayscale"
      local relPath = (gray and CUST_BACK_SPRITES_GRAY or CUST_BACK_SPRITES)[pick]
      if relPath then
        ctx.trueColor = not gray
        return mod.assets:path(relPath)
      end
    end
    return next(path, ctx)
  end)
  ...
  -- Custom trainer battle sprites --

    local TRAINER_SPRITES = {
    OPP_BUG_CATCHER = "bug_catcher.png",
    OPP_LASS = "lass.png",
    OPP_YOUNGSTER = "boy.png",
    OPP_JR_TRAINER_M = "juniorm.png",
    OPP_JR_TRAINER_F = "juniorf.png",
    OPP_MISTY = "misty.png",
    OPP_BROCK = "brock.png",
    OPP_FISHER = "fisherman.png",
    OPP_SAILOR = "navy.png",
    OPP_SCIENTIST = "scentist.png",
    OPP_HIKER = "hiker.png",
    OPP_SUPER_NERD = "nerd.png",
    OPP_ROCKER = "rocker.png",
    OPP_BIKER = "biker.png",
    OPP_POKEMANIAC = "pokemac.png",
    OPP_BURGLAR = "burgler.png",
    OPP_SWIMMER = "swimmer.png",
    OPP_BIRD_KEEPER = "birdkeeper.png",
    OPP_BLACKBELT = "birdkeeper.png",
    OPP_PROF_OAK = "oak.png",
    OPP_JUGGLER = "juggle.png",
    OPP_LT_SURGE = "sargent.png",
    OPP_SABRINA = "sabrina.png",
    OPP_BLAINE = "blaine.png",
    OPP_KOGA = "koga.png",
    OPP_GENTLEMAN = "gentleman.png",
    OPP_LANCE = "lance.png",
    OPP_ERIKA = "erika.png",
    OPP_CHIEF = "firebreather.png",
    OPP_BEAUTY = "beauty.png",
    OPP_PSYCHIC_TR = "psychic.png",
    OPP_BRUNO = "bruno.png",
    OPP_ROCKET = "rocket.png",
    OPP_COOLTRAINER_M = "rocket.png",
    OPP_COOLTRAINER_F = "rocket.png",
    OPP_TAMER = "tamer.png",
    OPP_CUE_BALL = "cue.png",
    OPP_GIOVANNI = "gio.png",
    OPP_ENGINEER = "engine.png",
    OPP_GAMBLER = "gamble.png",
    OPP_RIVAL1 = "gary1.png",
    OPP_RIVAL2 = "gary2.png",
    OPP_RIVAL3 = "gary3.png",
    OPP_LORELEI = "lorelei.png",
    OPP_AGATHA = "agatha.png",
    OPP_CHANNELER = "channeler.png",
  }

  local originalTrainerPalette = BattleState.trainerPalette
  BattleState.trainerPalette = function(data, trainer)
    if trainer and TRAINER_SPRITES[trainer.id]
        and mod.options:get("trainerColorMode") ~= "grayscale" then
      return nil
    end
    return originalTrainerPalette(data, trainer)
  end

  local originalTrainerPicPath = BattleState.trainerPicPath
  BattleState.trainerPicPath = function(data, trainer, oppClass, partyIndex)
    if trainer and mod.options:get("trainerColorMode") == "grayscale" then
        local bwFile = TRAINER_SPRITES[trainer.id]
        if bwFile then return mod.assets:path("assets/bw/" .. bwFile) end
    end
    return originalTrainerPicPath(data, trainer, oppClass, partyIndex)
  end
 
  mod.content.trainers:patch("OPP_BUG_CATCHER", {
    pic = mod.assets:path("assets/bug_catcher.png"),
  })
 
  mod.content.trainers:patch("OPP_LASS", {
    pic = mod.assets:path("assets/lass.png"),
  })

 mod.content.trainers:patch("OPP_YOUNGSTER", {
    pic = mod.assets:path("assets/boy.png"),
  })

 mod.content.trainers:patch("OPP_JR_TRAINER_M", {
    pic = mod.assets:path("assets/juniorm.png"),
  })

 mod.content.trainers:patch("OPP_JR_TRAINER_F", {
    pic = mod.assets:path("assets/juniorf.png"),
  })
 ...
 mod.content.trainers:patch("OPP_MISTY", {
    pic = mod.assets:path("assets/misty.png"),
  })

mod.content.trainers:patch("OPP_BROCK", {
    pic = mod.assets:path("assets/brock.png"),
  })

mod.content.trainers:patch("OPP_FISHER", {
    pic = mod.assets:path("assets/fisherman.png"),
  })

mod.content.trainers:patch("OPP_SAILOR", {
    pic = mod.assets:path("assets/navy.png"),
  })

mod.content.trainers:patch("OPP_SCIENTIST", {
    pic = mod.assets:path("assets/scentist.png"),
  })

mod.content.trainers:patch("OPP_HIKER", {
    pic = mod.assets:path("assets/hiker.png"),
  })

mod.content.trainers:patch("OPP_SUPER_NERD", {
    pic = mod.assets:path("assets/nerd.png"),
  })

mod.content.trainers:patch("OPP_ROCKER", {
    pic = mod.assets:path("assets/rocker.png"),
  })

mod.content.trainers:patch("OPP_BIKER", {
    pic = mod.assets:path("assets/biker.png"),
  })

mod.content.trainers:patch("OPP_POKEMANIAC", {
    pic = mod.assets:path("assets/pokemac.png"),
  })

mod.content.trainers:patch("OPP_BURGLAR", {
    pic = mod.assets:path("assets/burgler.png"),
  })

mod.content.trainers:patch("OPP_SWIMMER", {
    pic = mod.assets:path("assets/swimmer.png"),
  })

mod.content.trainers:patch("OPP_BIRD_KEEPER", {
    pic = mod.assets:path("assets/birdkeeper.png"),
  })
...
mod.content.trainers:patch("OPP_PROF_OAK", {
    pic = mod.assets:path("assets/oak.png"),
  })

mod.content.trainers:patch("OPP_JUGGLER", {
    pic = mod.assets:path("assets/juggle.png"),
  })

mod.content.trainers:patch("OPP_LT_SURGE", {
    pic = mod.assets:path("assets/sargent.png"),
  })

mod.content.trainers:patch("OPP_SABRINA", {
    pic = mod.assets:path("assets/sabrina.png"),
  })

mod.content.trainers:patch("OPP_BLAINE", {
    pic = mod.assets:path("assets/blaine.png"),
  })

mod.content.trainers:patch("OPP_KOGA", {
    pic = mod.assets:path("assets/koga.png"),
  })

mod.content.trainers:patch("OPP_GENTLEMAN", {
    pic = mod.assets:path("assets/gentleman.png"),
  })

mod.content.trainers:patch("OPP_LANCE", {
    pic = mod.assets:path("assets/lance.png"),
  })

mod.content.trainers:patch("OPP_ERIKA", {
    pic = mod.assets:path("assets/erika.png"),
  })

mod.content.trainers:patch("OPP_CHIEF", {
    pic = mod.assets:path("assets/firebreather.png"),
  })

mod.content.trainers:patch("OPP_BEAUTY", {
    pic = mod.assets:path("assets/beauty.png"),
  })

mod.content.trainers:patch("OPP_PSYCHIC_TR", {
    pic = mod.assets:path("assets/psychic.png"),
  })

mod.content.trainers:patch("OPP_BRUNO", {
    pic = mod.assets:path("assets/bruno.png"),
  })

mod.content.trainers:patch("OPP_ROCKET", {
    pic = mod.assets:path("assets/rocket.png"),
  })
...
mod.content.trainers:patch("OPP_TAMER", {
    pic = mod.assets:path("assets/tamer.png"),
  })

mod.content.trainers:patch("OPP_CUE_BALL", {
    pic = mod.assets:path("assets/cue.png"),
  })

mod.content.trainers:patch("OPP_GIOVANNI", {
    pic = mod.assets:path("assets/gio.png"),
  })

mod.content.trainers:patch("OPP_ENGINEER", {
    pic = mod.assets:path("assets/engine.png"),
  })

mod.content.trainers:patch("OPP_GAMBLER", {
    pic = mod.assets:path("assets/gamble.png"),
  })

mod.content.trainers:patch("OPP_RIVAL1", {
    pic = mod.assets:path("assets/gary1.png"),
  })

mod.content.trainers:patch("OPP_RIVAL2", {
    pic = mod.assets:path("assets/gary2.png"),
  })

mod.content.trainers:patch("OPP_RIVAL3", {
    pic = mod.assets:path("assets/gary3.png"),
  })

mod.content.trainers:patch("OPP_LORELEI", {
    pic = mod.assets:path("assets/lorelei.png"),
  })

mod.content.trainers:patch("OPP_AGATHA", {
    pic = mod.assets:path("assets/agatha.png"),
  })

mod.content.trainers:patch("OPP_CHANNELER", {
    pic = mod.assets:path("assets/channeler.png"),
  })

-- Oak/Red/Gary/ opening-sequence portraits --

--local OakSpeech = require("src.ui.OakSpeech")
--local OAK_INTRO_TRUECOLOR = mod.assets:image("assets/oak.png")
--local OAK_INTRO_GRAYSCALE = mod.assets:image("assets/bw/oak.png")
        
--local function isOakIntroPic(desc)
--  return desc == "oak"
--    or (type(desc) == "table" and desc.type == "trainer" and desc.id == "OPP_PROF_OAK")
--end

--local originalResolvePic = OakSpeech.resolvePic
--OakSpeech.resolvePic = function(game, desc, speech)
--if isOakIntroPic(desc) then
-- local gray = mod.options:get("trainerColorMode") == "grayscale"
--  local img = gray and OAK_INTRO_GRAYSCALE or OAK_INTRO_TRUECOLOR
--  if img then return img, false, not gray end
--  return originalResolvePic(game, desc, speech)


--local RIVAL_INTRO_TRUECOLOR = mod.assets:image("assets/gary1.png")
--local RIVAL_INTRO_GRAYSCALE = mod.assets:image("assets/bw/gary1.png")

--if isRivalIntroPic(desc) then  
-- local img = gray and RIVAL_INTRO_GRAYSCALE or RIVAL_INTRO_TRUECOLOR
-- if img then return img, false, not gray end
--end 

--mod.hooks:wrap("player.sprite", function(next, path, ctx)
--  if ctx.side == "front" and ctx.kind == "intro" then
--    local gray = mod.options:get("trainerColorMode") == "grayscale"
--    ctx.trueColor = not gray
--    return mod.assets:path(gray and "assets/bw/red_pose.png" or "assets/red_pose.png")
--  end
--  return next(path, ctx)
--end)

local AnimPlayer = require("src.battle.AnimPlayer")
local ANIM_TILESHEET_FILES = {
  [0] = "move_anim_0.png",
  [1] = "move_anim_1.png",
}

local originalSheetImage = AnimPlayer.sheetImage
AnimPlayer.sheetImage = function(self, ts)
  local file = ANIM_TILESHEET_FILES[ts]
  if file then
    local cached = self.images[ts]
    if cached ~= nil then return cached or nil end
    local gray = mod.options:get("animColorMode") == "grayscale"
    local relPath = (gray and "anims/bw/" or "anims/") .. file
    local ok, img = pcall(love.graphics.newImage, mod.assets:path(relPath))
    self.images[ts] = (ok and img) or false
    return self.images[ts] or nil
  end
  return originalSheetImage(self, ts)
end

local function animIsTrueColor()
  return mod.options:get("animColorMode") ~= "grayscale"
end

local originalDraw = AnimPlayer.draw
AnimPlayer.draw = function(self, colorFn)
  if animIsTrueColor() then colorFn = nil end
  return originalDraw(self, colorFn)
end
